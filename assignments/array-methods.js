...
let fullNames = [];
runners.forEach(list => fullNames.push(`${list.first_name} ${list.last_name}`));
//console.table(fullNames); 

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.

let firstNamesAllCaps = runners.map(list => list.first_name.toUpperCase());
// console.table(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.

let runnersLargeSizeShirt = runners.filter(list => list.shirt_size === "L");
//console.table(runnersLargeSizeShirt); // This will give us a 'list' of everyone needing a 'L'


// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.

let ticketPriceTotal = runners.reduce((listTotal, runner) => runner.donation += listTotal, 0)
...
// Problem 1
// The event director wants to send a thank you letter to all the donators that donated more than $100. 

let topDonators = runners.filter(donatorList => donatorList.donation >=100);
//console.table(topDonators);

// Problem 2
//The event director wants to send an email to all the donators that have a shirt size of M. Because they have ran out of that shirt size.

let mediumDonators = runners.filter(mediumDonators => mediumDonators.shirt_size === 'M');
//console.table(mediumDonators);

// Problem 3
// The event director want to see how many people work in the company Mycat because they forgot there running shoes.

let myCatRunners = runners.filter(myCatRunners => myCatRunners.company_name === 'Mycat');
console.table(myCatRunners);